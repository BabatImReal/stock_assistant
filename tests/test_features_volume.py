...
def test_every_configured_measure_is_registered():
    # The shipped config and the code must not drift apart -- in ANY
    # registry. A measure present in features.yaml and in none would be
    # enabled and never computed.
    from vnstock_research.features import MARKET_REGISTRY, SECTOR_REGISTRY

    known = set(REGISTRY) | set(MARKET_REGISTRY) | set(SECTOR_REGISTRY)
