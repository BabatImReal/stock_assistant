...
| `scripts/snapshot_industry.py` | Writes today's dated ICB snapshot from vnstock VCI (idempotent per day; `--dry-run`) and prints the KBS rough cross-check and our coverage | pandas, vnstock, `data.sectors`, `data.db` |
...
| `tests/test_data_integrity.py` | Live-database checks, skipped when no DB: `date_shifted` really is written, no weekend bars survive, the price-limit SQL uses the rule in force, the calendar matches `bar_raw` (failed with 7,352 drifted exchange-days before the 2026-09-23 fix), the latest liquid set all traded that session, and the latest industry snapshot labels ≥ 99% of the symbols that traded that session | psycopg |
