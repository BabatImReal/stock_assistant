...
| `data/exchanges.py` | **Dated exchange membership.** `kbs_rows` (KBS listing_date cache → open-ended spans; OTC/empty give none), `cafef_transfer_rows` (G4 Class A spans), `rebuild` (TRUNCATE + insert, like `rebuild_trading_day`), `load`, `resolve` (pandas: exchange + `exchange_unknown`, CafeF transfer spans first, then KBS, else the filed exchange borrowed), `RESOLVE_JOIN_SQL` (the same in SQL, for the gate) | pandas, `data.db` |
