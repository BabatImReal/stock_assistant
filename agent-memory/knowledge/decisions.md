...
## Session 2026-09-23-03 run 26 — STRUCTURAL features + batch `structural_1`, branch `features/structural-features`

| Date | Decision | Reason | Rejected |
| --- | --- | --- | --- |
| 2026-09-25 | Ben chose: merge `features/new-hypotheses` into main FIRST (fast-forward to `dfabd5b`, branch deleted), then cut `features/structural-features` from main | The batch runner + complements_1 results lived only there; one branch | cutting from the old main (no batch engine); stacking |
| 2026-09-25 | **The structural measures live OUTSIDE features/ (`src/vnstock_research/structural.py`, config `config/rules/structural.yaml`)** | The stored fingerprint and returns refuse to load when any code under data/, features/ or patterns/ changes; the registered featureset (holdout, neighbours, scan) had to stay loadable. Verified: the old featureset still loads | adding them to features/ (would stale every stored artifact) |
| 2026-09-25 | **The structural featureset closes the drift hole via params**: each structural measure's params carry `code` = sha of structural.py, so a changed rule = a different FeatureSet fingerprint | fingerprint.py (in patterns/) could not be edited without staling the old store | no code tracking for the new measures |
| 2026-09-25 | Measures: rs_index_20d/60d/120d (stock minus VN-Index return over N sessions; lookback N); down_day_rs_20d (mean stock-minus-index return on index-down days in 20; every index day must be known; >= 3 down days); price_vs_ma_200; ma_200_slope (20-session change; lookback 219); trend_stage 1-4 (2 = close>MA50>MA200 & MA200 rising, 4 = mirror & falling, 3 = rising not 2, 1 = not rising not 4). Index close joined per symbol by date, never filled | Ben's list; base quality SKIPPED (does not fit cleanly: many free parameters); sector RS SKIPPED (labels are current-only, flagged B3) | VCP/base depth now; sector RS |
| 2026-09-25 | The structural fingerprint starts at 2012-01-01 like the registered one, so the registered measures' values are IDENTICAL in both; the price cost is warm-up: MA200/stage/RS120 are NaN for most of 2012 | Same price base, same values | loading 2011 for warm-up (would change the registered measures' 2012 values) |
| 2026-09-25 | **Batch `structural_1` (hash `1ff6d6de2472c886`, N 3,024)**: every trigger × exactly ONE of 8 structural conditions (rs_20/60/120_strong > 0, resists_down_days > 0, stage_1..4) × (none or ONE registered condition) × k 3,5. RS on its strong side only (the hypothesis is strength); the test is two-sided; the stage covers both trend directions; price_vs_ma_200 / ma_200_slope enter only via the stage | Keeps N below complements_1's 3,864 | complements of the RS conditions (N 4,536); structural × structural pairs |
| 2026-09-25 | Same gates as complements_1 (gross hit, BH q 0.10 over this N + abs(edge) ≥ 3 pts; validate same sign + gross > 0). "Materially stronger" benchmark = the six AND the complements_1 candidates (from its stored validate results), same gross basis | Ben | — |
